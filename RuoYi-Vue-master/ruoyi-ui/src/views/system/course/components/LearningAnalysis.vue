...
export default {
  ...
  methods: {
    ...
    extractTagsFromRecommendations(itemList) {
      const kpSet = new Set()
      itemList.forEach(item => {
        if (item.relatedKpIds) {
          const ids = item.relatedKpIds.split(',').map(id => id.trim())
          ids.forEach(id => kpSet.add(id))
        }
      })
      this.statusTags = Array.from(kpSet).map(kpId => ({ kpName: `知识点${kpId}` }))
    },
    ...
    extractRecommendAction(reason, type) {
      if (!reason) return '学习建议'
      const match = reason.match(/^(.{0,20}?)[:：]/)
      return match ? match[1] : (type || '学习建议')
    },
    ...
    formatKpIds(kpIds, item) {
      if (!kpIds) return '无'
      return kpIds.split(',').slice(0, 3).join(', ') + (kpIds.split(',').length > 3 ? '...' : '')
    },
